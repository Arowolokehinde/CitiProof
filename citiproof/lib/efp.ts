...
export class EFPService {
  ...
  constructor(chainId: number = sepolia.id) {
    ...
    console.log(`[EFP SERVICE] Initialized for ${chainId === mainnet.id ? 'MAINNET' : 'TESTNET'} (Chain ID: ${chainId})`)
  }
  ...
  private async fetchRealEFPData(address: string): Promise<EFPProfile> {
    ...
    if (registryAddress && registryAddress !== '0x0000000000000000000000000000000000000000') {
      try {
        ...
        const balance = await registryContract.read.balanceOf([address as `0x${string}`])
      ...
      console.warn('[EFP SERVICE] No valid contract address configured - EFP contracts not yet deployed according to research')
    }
    ...
    if (apiData || contractData) {
      ...
      const verificationLevel = hasEFPList 
        ? (followerCount > 50 ? 'verified' : 'basic')
        : 'none'

      console.log(`[EFP SERVICE] Returning REAL DATA: hasEFP=${hasEFPList}, followers=${followerCount}, following=${followingCount}`)
